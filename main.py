...
from utils import all_node_plot, per_node_barplot, per_node_plot, per_node_plotly_plot, plot_cdf, per_node_rate_plot, \
    scaled_rate_plot
...
#     html.Div(id='updates', children=2),
...
def simulate(per_node_result_keys, dirstr):
    ...
    """
    # STEP = The time step used in the simulation.
    ...
    print("PacketsInTransit", PacketsInTransit)
    ...
    for mc in range(MONTE_CARLOS):
        ...
        # GMatrix = np.asarray(nx.to_numpy_matrix(G))
        # filename = "adjacency_matrix_of_G.txt"
        #
        # # Write the adjacency matrix to the file
        # np.savetxt(filename, GMatrix, fmt='%d', delimiter=',')

        ChannelDelays = 0.05 * np.ones((NUM_NODES, NUM_NODES)) + 0.1 * np.random.rand(NUM_NODES, NUM_NODES)
        AdjMatrix = np.multiply(1 * np.asarray(nx.to_numpy_matrix(G)), ChannelDelays)

        # Save the AdjMatrix to a file named 'adjacency_matrix.txt'
        # np.savetxt('adjacency_matrix.txt', AdjMatrix, fmt='%f', delimiter=',')
        ...
        # nx.draw(G, pos, with_labels=True,nodelist=range(NUM_NODES),
        #            node_color=[MODE_COLOUR_MAP[MODE[NodeID]] for NodeID in range(NUM_NODES)],
        #            node_size=200,
        #        alpha=0.8, linewidths=1,
        #         font_size=12)
        # plt.title("Zachary Karate Club Network Topology")
        # plt.show()
        ...
        for i in range(TimeSteps):
            ...
            if 100 * i / TimeSteps % 10 == 0:
                ...
                # print("PacketsInTransit in round ", i, " = ", PacketsInTransit)
            # discrete time step size specified by global variable STEP
            T = STEP * i
            ...
            for NodeID, Node in enumerate(Net.Nodes):
                ...
                per_node_results['ReadyPackets NonMalNeighb'][mc][i, NodeID] = len(
                    NonMalNeighb.Inbox.ReadyPackets[NodeID])
                per_node_results['Dropped Messages'][mc][i, NodeID] = sum(
                    [len(Node.DroppedPackets[i]) for i in range(NUM_NODES)])
                ...
                per_node_results['Number of Honest Tips'][mc][i, NodeID] = sum(
                    [len(Node.NodeTipsSet[i]) for i in range(NUM_NODES) if MODE[i] < 3])
        ...
        for i in range(int(TimeSteps / 100)):
            s = STEP * 100
            delays = [Net.MsgDelays[j] for j in Net.MsgDelays if
                      s * int(Net.DissemTimes[j] / s) == i * s and MODE[Net.MsgIssuer[j]] < 3]
            if delays:
                MeanDelay[mc][i] = sum(delays) / len(delays)
            confDelays = [Net.ConfTimes[j] - Net.Nodes[0].Ledger[j].IssueTime for j in Net.ConfTimes if
                          s * int(Net.ConfTimes[j] / s) == i * s]
            if confDelays:
    ...
    # plt.axis('off')
    # plt.savefig(dirstr + '/plots/Graph.png', bbox_inches='tight')
...
def plot_results(dirstr, per_node_result_keys):
    ...
    avgTP = np.concatenate((np.zeros((avg_window, NUM_NODES)), (data[avg_window:, :] - data[:-avg_window, :]))) / (
                avg_window * STEP)
    ...
    scaled_rate_plot(per_node_results['Number of Disseminated Messages'], 'Time (sec)', 'Dissemination Rate', '',
                     dirstr)
    ...
    per_node_rate_plot(per_node_results['Number of Disseminated Messages'], 'Time (sec)', 'Dissemination Rate', '',
                       dirstr)
    ...
    per_node_plot(avgEligibleDelays, 'Time (sec)', 'Age of Messages Becoming Eligible', '', dirstr, avg_window=20,
                  step=1)
    ...
    all_node_plot(per_node_results['Number of Unconfirmed Messages'].sum(axis=1), 'Time (sec)',
                  'Number of Unconfirmed Messages', '', dirstr + '/plots/AllUnconfirmed.png')
